...
public class StringFormatter {
     ...
     * @return escaped string
     */
    private static String escapeQuotesForXpath (String originalString, String quote) {
        String formattedString = "";
        if (StringUtils.isNotBlank(originalString) && StringUtils.isNotBlank(quote)) {
            ...
            if (originalString.startsWith(quote)) {
                formattedString += xpathDelimiterForQuote + quote + xpathDelimiterForQuote;
                formattedString += ",";
            }
            String[] parts = originalString.split(quote);
            if (parts.length > 1) {
                for (int i = 0; i < parts.length; i++) {
                    formattedString += xpathDelimitter + parts[i] + xpathDelimitter;
                    if (i < parts.length - 1) {
                        formattedString += ",";
                        formattedString += xpathDelimiterForQuote + quote + xpathDelimiterForQuote;
                        formattedString += ",";
                ...
                formattedString = xpathDelimitter + originalString + xpathDelimitter;
            }
        }
     ...
     * 
     * @param stringToBeDifferent
     *            string we want to have another different.
     * @return a different random string.
     */
    public static String getARandomAndDifferentStringOf (String stringToBeDifferent) {
     ...
     * @return a different random string.
     */
    public static String getARandomAndDifferentStringOf (List<String> stringsToBeDifferent) {
